...
public class CartSyncHandler implements HttpHandler {
    // Thread-safe map: cashierId -> (productId -> quantity)
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            if ("POST".equalsIgnoreCase(method)) {
                ...
                // Simple JSON parser for cart items (since we don't have Jackson/Gson)
                // Cart format in JSON is e.g. [{"productId":"P1001","quantity":2}, ...]
                Map<String, Integer> cartMap = new HashMap<>();
                
                // Let's locate the "cart" array in the JSON body
                ...
                // Clear cart lock for cashier (e.g. on logout)
    ...
    /**
     * Get the total locked quantity of a product by all cashiers *except* the requesting cashier.
     */

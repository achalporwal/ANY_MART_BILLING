    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            if ("POST".equalsIgnoreCase(method)) {
                ...
                // Parse items array: [{"productId":"P1001","quantity":2}, ...]
                ...
                // Get all bills — filter options:
                //   ?cashierId=X            → today's bills for that cashier
                //   ?date=YYYY-MM-DD        → all bills on that specific date
                //   ?month=M&year=YYYY      → all bills in that month
                //   (no params)             → all bills ever
                ...
                if (cashierFilter != null && !cashierFilter.trim().isEmpty()) {
                    // Cashier session log — today only
                    ...
                    // Admin: specific month+year filter
                    try {
